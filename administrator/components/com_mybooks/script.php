...
class com_mybooksInstallerScript
{
  /**
  ...
  function preflight($type, $parent) 
  {
    ...
    echo '<p>'.JText::_('COM_MYBOOKS_INSTALLING_COMPONENT_VERSION').$this->release;
    echo '<br />'.JText::_('COM_MYBOOKS_CURRENT_JOOMLA_VERSION').$jversion->getShortVersion().'</p>';
	...
	JFactory::getApplication()->enqueueMessage(JText::_('COM_MYBOOKS_UPDATE_INCORRECT_VERSION').$rel, 'error');
	return false;
      }
    }
  ...
  function install($parent) 
  {
    ...
    $catData = array('id' => 0, 'parent_id' => 1, 'level' => 1, 'path' => 'mybooks',
		     'extension' => 'com_mybooks', 'title' => 'My Books',
		     'alias' => 'mybooks', 'description' => '<p>Default category</p>',
  ...
  function uninstall($parent) 
  {
    ...
    $query->delete('#__content_types')
	  ->where('type_alias="com_mybooks.book" OR type_alias="com_mybooks.category"');
    $db->setQuery($query);
    $db->query();
  }
  ...
  function postflight($type, $parent) 
  {
    if($type == 'install') {
      ...
      $form = new JForm('mybooks_config');
      // N.B: The third parameter must be set or the xml file won't be loaded.
      $form->loadFile(JPATH_ROOT.'/administrator/components/com_mybooks/config.xml', true, '/config');
      $JsonValues = '';
      ...
      $query->where('element='.$db->Quote('com_mybooks').' AND type='.$db->Quote('component'));
      $db->setQuery($query);
      $db->query();
      ...
      // Informations about the My Books book items.
      $columns = array('type_title', 'type_alias', $db->quoteName('table'), 'field_mappings', 'router');
      $query->clear();
      $query->insert('#__content_types');
      $query->columns($columns);
      $query->values($db->Quote('My Books').','.$db->Quote('com_mybooks.book').','.
$db->Quote('{"special":{"dbtable":"#__mybooks_book","key":"id","type":"Book","prefix":"MybooksTable","config":"array()"},"common":{"dbtable":"#__ucm_content","key":"ucm_id","type":"Corecontent","prefix":"JTable","config":"array()"}}').','.
...
$db->Quote('MybooksHelperRoute::getBookRoute'));
      $db->setQuery($query);
      $db->query();

      // Informations about the My Books category items.
      $query->clear();
      $query->insert('#__content_types');
      $query->columns($columns);
      $query->values($db->Quote('My Books Category').','.$db->Quote('com_mybooks.category').','.
...
$db->Quote('{"common":{"core_content_item_id":"id","core_title":"title","core_state":"published","core_alias":"alias","core_created_time":"created_time","core_modified_time":"modified_time","core_body":"description","core_hits":"hits","core_publish_up":"null","core_publish_down":"null","core_access":"access","core_params":"params","core_featured":"null","core_metadata":"metadata","core_language":"language","core_images":"null","core_urls":"null","core_version":"version","core_ordering":"null","core_metakey":"metakey","core_metadesc":"metadesc","core_catid":"parent_id","core_xreference":"null","asset_id":"asset_id"},"special":{"parent_id":"parent_id","lft":"lft","rgt":"rgt","level":"level","path":"path","extension":"extension","book":"book"}}').','.
$db->Quote('MybooksHelperRoute::getCategoryRoute'));
      $db->setQuery($query);
      $db->query();
    }
  }
